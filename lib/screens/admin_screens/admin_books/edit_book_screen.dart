// lib/screens/edit_book/edit_book_screen.dart
...
class _EditBookScreenState extends State<EditBookScreen> {
  ...
  void initState() {
    super.initState();
    // Inicializar con al menos un campo para autor
    _addAuthorField();
  }
  ...
  void _populateFields(BookDto book) {
    ...
    for (String genre in book.genres) {
      ...
      _genreControllers.add(controller);
    }
    ...
    // Asegurar al menos un campo de autor
    if (_authorControllers.isEmpty) {
      _addAuthorField();
    }
  }
  ...
  void _saveBook(EditBookBloc bloc, BookDto originalBook) {
    if (_formKey.currentState!.validate()) {
      ...
      final updatedBook = BookDto(
        ...
        totalReviews: originalBook.totalReviews,
        createdAt: originalBook.createdAt,
        updatedAt: DateTime.now(),
      );

      bloc.add(EditBookUpdate(book: updatedBook));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title:
            const Text('Editar Libro', style: TextStyle(color: Colors.white)),
        ...
        actions: [
          BlocBuilder<EditBookBloc, EditBookState>(
            builder: (context, state) {
              if (state is EditBookLoaded) {
                return IconButton(
                  icon: const Icon(Icons.save, color: Colors.white),
                  onPressed: () => _saveBook(
                    context.read<EditBookBloc>(),
                    state.book,
                  ),
                );
              }
      ...
      body: BlocConsumer<EditBookBloc, EditBookState>(
        listener: (context, state) {
          if (state is EditBookLoaded) {
            _populateFields(state.book);
          } else if (state is EditBookUpdateSuccess) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
        ...
        builder: (context, state) {
          ...
          if (state is EditBookError && state is! EditBookLoaded) {
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: () => context
                        .read<EditBookBloc>()
                        .add(EditBookLoad(bookId: widget.bookId)),
          ...
          if (state is EditBookLoaded || state is EditBookUpdating) {
            return Stack(
              children: [
                _buildForm(),
                if (state is EditBookUpdating)
                  Container(
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () {
                  final bloc = context.read<EditBookBloc>();
                  final state = bloc.state;
                  if (state is EditBookLoaded) {
                    _saveBook(bloc, state.book);
                  }
                },
